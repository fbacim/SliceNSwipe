...
public class BubbleZoomv2 {
	...
	public bool Update (Frame frame, List<GameObject> goHandList, List<GameObject> goFingerList) {
		...
		{
			// radius of the bubble is determined by the size of the selection box and the angle of the two fingers
			float angle = ((fl[0].TipPosition-hl[0].PalmPosition).AngleTo(fl[1].TipPosition-hl[0].PalmPosition)*57.2957795F);
			float selectionVolumeScale = angle*Mathf.Max(new float[]{pointCloud.Size().x,pointCloud.Size().y,pointCloud.Size().z})/60.0F;
